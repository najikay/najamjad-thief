...
- [ ] **T-2434** (P1) Raise `play_match(wait_seconds=120)` — two teams agreeing a time do not both start within two minutes, and the wait is now a real readiness check rather than a TCP probe that passed instantly. Also USE its result: the return value is currently discarded, so a timed-out wait proceeds into `play_series` and dies at the handshake instead of saying the opponent never arrived — DoD: longer default, and a clean abort naming the opponent
