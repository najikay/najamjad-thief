...
- [x] **T-2474** (P0) Feed capture claims and barrier declarations into the belief. Against a peer that sends no scent and no hints the belief stayed uniform, `_cop_cell` correctly refused it, and the thief fell back to the weighted sum that lost three games — so the exact solve and the distance-2 invariant never executed against uoh-sqak at all. The archive shows the result: four distinct cells in eleven steps across g02/g04/g06. A claim is the cop's exact cell (rules 21-22), a barrier is a five-cell reach set (rules 15-16); 289 disclosures in one series, all discarded — DoD: 35/35 survival against their recorded line with a silent peer, replayed through the real `absorb_turn`/`decay_after_full_turn` [FR-STR-5]
