...
- [ ] **T-2485** (P1) Shrink the blast radius of a stalled mini-game from two games to one. Measured in `test_series_survives_a_stall.py`: a send storm kills the game it lands in **and the one after**, because when we abandon mid-mini-game the peer does not know — it is still waiting on a turn and burns its own watchdog before both sides resynchronise at the next boundary. Two is bounded and survivable where six was not, but it is still one more game than it needs to be — DoD: abandonment sends a control message naming the sub-game we are giving up, so the peer closes it on the same reason at the same time rather than timing out into the next one
