...
| `strength.level` | `full` | One mode since 2026-08-22: every recognised spelling (`full`, and the legacy `practice`/`sandbagged`) resolves to full, so a stale config cannot change how we play; an unknown value refuses at startup so a typo cannot hide. |
