...
from najamjad_agent.reporting.sibling_merge import assemble, partial_path, write_partial
...
def test_a_half_left_by_an_earlier_attempt_is_refused(tmp_path, table):
    """`game_uid` is deterministic, so a restarted series reuses the filename.

    `derive_game_ids` hashes the terms and the two group ids and nothing else —
    by design, so both peers compute the same value independently. A series
    that stalls and is replayed therefore writes to the same partial path. The
    second attempt must not merge the first attempt's games: one report
    describing games from two runs is self-contradictory, and rules 33-35 void
    both teams for that.
    """
    sibling = tmp_path / "najamjad-thief"
    here = tmp_path / "najamjad-cop"
    here.mkdir()
    stale_games, stale_outcomes = _half([1, 3, 5], Role.THIEF)
    write_partial(sibling, "uid-1", Role.THIEF, stale_games, stale_outcomes)
    # This attempt began after that half was written.
    ours_games, ours_outcomes = _half([2, 4, 6], Role.COP)
    ...
    assert [game["sub_game"] for game in games] == [2, 4, 6]
    assert "series.sibling_half_stale" in [event["event"] for event in seen]
